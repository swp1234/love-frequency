...
function updateTestCount() {
    ...
    if (c > 0) {
        el.textContent = `${c.toLocaleString()}${i18n?.t('premium.participantsCount') || 'people have participated!'}`;
...
function showLoading() {
    ...
    const messages = [
        i18n?.t('premium.loadingMessages.msg1') || 'Detecting love frequency...',
        i18n?.t('premium.loadingMessages.msg2') || 'Analyzing emotional frequency...',
        i18n?.t('premium.loadingMessages.msg3') || 'Interpreting soul resonance pattern...',
        i18n?.t('premium.loadingMessages.msg4') || 'Tuning your unique love frequency...',
        i18n?.t('premium.loadingMessages.msg5') || 'Finalizing frequency...'
    ];
...
function toggleFrequency() {
    ...
    if (isPlaying) {
        stopFrequency();
        btn.textContent = `🎧 ${resultData.freq}Hz ${i18n?.t('premium.frequencyButtons.play') || 'Play Frequency'}`;
        btn.classList.remove('playing');
    } else {
        playFrequency(resultData.freq);
        btn.textContent = `⏹️ ${i18n?.t('premium.frequencyButtons.stop') || 'Stop Frequency'}`;
...
function displayPremiumContent() {
    ...
    // Compatibility chart
    let compatHTML = `<div class="detail-section"><h3>${i18n?.t('premium.compatibilityChart') || '💞 Frequency Compatibility Chart'}</h3><div class="compat-grid">`;
    ...
    sortedCompat.forEach(c => {
        const level = c.score >= 90 ? (i18n?.t('premium.perfectMatch') || 'Perfect') :
    ...
    // Weekly routine
    let routineHTML = `<div class="detail-section"><h3>${i18n?.t('premium.weeklyRoutine') || '📅 Couple Weekly Routine'}</h3><ul>`;
    advice.weeklyRoutine.forEach(r => { routineHTML += `<li>${r}</li>`; });
    routineHTML += '</ul></div>';

    // Date ideas
    let dateHTML = `<div class="detail-section"><h3>${i18n?.t('premium.dateIdeas') || '💑 Recommended Dates'}</h3><ul>`;
    advice.dateIdeas.forEach(d => { dateHTML += `<li>${d}</li>`; });
    dateHTML += '</ul></div>';

    // Music recommendation
    let musicHTML = `<div class="detail-section"><h3>${i18n?.t('premium.musicRec') || '🎵 Recommended Music'}</h3><p class="music-rec">${advice.musicRec}</p></div>`;
...
function shareResult() {
    const text = (i18n?.t('premium.shareText') || `💕 My Love Frequency: {freq}Hz\n{emoji} {title}\n"{subtitle}"\n\nWhat's your love frequency?\n👉 https://dopabrain.com/love-frequency/\n\n#LoveFrequency #DatingTest #LoveFrequencyTest`)
    ...
    if (navigator.share) {
        navigator.share({ title: i18n?.t('premium.shareTitle') || 'Love Frequency Test', text, url: 'https://dopabrain.com/love-frequency/' }).catch(() => {});
    } else {
        navigator.clipboard.writeText(text).then(() => alert(i18n?.t('result.resultSharedMessage') || 'Result copied!')).catch(() => {});
    }
...
function generateShareImage() {
    ...
    ctx.fillText(i18n?.t('premium.youLoveFrequency') || 'My Love Frequency is', w / 2, 160);
    ...
    ctx.font = '400 26px -apple-system, sans-serif';
    ctx.fillText(i18n?.t('premium.testYourFrequency') || "What's your love frequency? 👉 Love Frequency Test", w / 2, 900);
